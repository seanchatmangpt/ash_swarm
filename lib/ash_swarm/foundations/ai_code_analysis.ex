defmodule AshSwarm.Foundations.AICodeAnalysis do
      ...
      %{
        ...
        "properties" => %{
          ...
          "location" => %{
            "type" => "string",
            "description" =>
              "Where in the code the optimization can be applied (function/method/line)"
          },
          "severity" => %{
            "type" => "string",
    ...
    try do
      ...
      case Igniter.Project.Module.find_module(igniter, module) do
        {:ok, {_igniter, _source, zipper}} ->
          ...
          function_source =
            extract_function_source(zipper, function_name, arity) ||
        ...
        {:error, reason} ->
          ...
          Logger.warning(
            "Could not find module #{inspect(module)}: #{inspect(reason)}. Using simulated function source."
          )
      ...
      e ->
        Logger.error(
          "Failed to analyze function #{function_name}/#{arity} in module #{inspect(module)}: #{inspect(e)}"
        )
  ...
  def analyze_source_code(source_code, _context, options \\ []) do
    ...
    focus_areas =
      Keyword.get(options, :focus_areas, [:performance, :readability, :maintainability])
