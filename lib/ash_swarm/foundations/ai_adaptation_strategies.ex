defmodule AshSwarm.Foundations.AIAdaptationStrategies do
      ...
      %{
        ...
        "properties" => %{
          "performance" => %{
            "type" => "string",
            "description" =>
              "Expected performance improvement (e.g., '30% faster for large inputs')"
          },
          "maintainability" => %{
            "type" => "string",
    ...
    def to_json_schema do
      %{
        ...
        "properties" => %{
          ...
          "code" => %{
            "type" => "string",
            "description" => "Code for the alternative optimization"
          },
          "rationale" => %{
            "type" => "string",
            "description" => "Rationale for the alternative optimization"
          },
          "risk_assessment" => %{
            "type" => "string",
            "description" => "Risk assessment for the alternative optimization"
          }
        },
            ...
            expected_improvements:
              (result.expected_improvements &&
                 %ExpectedImprovements{
                   ...
                   maintainability:
                     Map.get(result.expected_improvements || %{}, :maintainability, ""),
  ...
  @spec suggest_alternative_optimizations(String.t(), String.t(), map(), keyword()) ::
          {:ok, list(map())} | {:error, any()}
  def suggest_alternative_optimizations(
        original_code,
        failed_optimization,
        failure_details,
        options \\ []
  ...
  defp with_retry(max_retries, initial_backoff, func, current_retry)
       when current_retry > 0 and current_retry < max_retries do
    # Retry attempts
    case func.() do
      {:error, :rate_limit, error_msg} ->
        ...
        Logger.info(
          "Rate limit hit: #{error_msg}, retrying in #{trunc(backoff_ms)}ms (attempt #{current_retry}/#{max_retries})"
        )
  ...
  defp with_retry(max_retries, _initial_backoff, _func, current_retry)
