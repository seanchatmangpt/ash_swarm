defmodule AshSwarm.Foundations.UsageStats do
  ...
  def handle_cast({:update_stats, module, action, context}, state) do
    ...
    new_stats = %{
      ...
      execution_times:
        if(execution_time,
          do: [execution_time | current_stats.execution_times],
          else: current_stats.execution_times
        ),
      avg_exec_time:
        if(execution_time,
          do:
            calculate_avg(current_stats.avg_exec_time, current_stats.call_count, execution_time),
          else: current_stats.avg_exec_time
        ),
      # Keep only the last 100 contexts
      contexts: [context | current_stats.contexts] |> Enum.take(100)
    }
  ...
  def handle_call({:most_used_functions, module, limit}, _from, state) do
    ...
    module_functions =
      Enum.filter(state.stats, fn {{mod, _action}, _stats} ->
        mod == module
      end)
