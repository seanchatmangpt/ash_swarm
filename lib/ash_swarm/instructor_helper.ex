defmodule AshSwarm.InstructorHelper do
  ...
  * `{:error, reason}` - Error information
  ...
  defp extract_documentation_from_complex_code(text) do
    ...
    [
      (if function_docs != "", do: "Function Documentation:\n\n" <> function_docs <> "\n\n", else: ""),
      (if module_doc != "", do: "Module Documentation:\n\n" <> module_doc <> "\n\n", else: ""),
      (if examples != "", do: "Usage Examples:\n\n" <> examples <> "\n\n", else: ""),
      (if algorithm_explanations != "", do: "Algorithm Explanations:\n\n" <> algorithm_explanations, else: "")
  ...
  defp extract_function_documentation(text) do
    ...
    initial_docs = Regex.scan(~r/Function: `([^`]+)`[\s\n]*[\-\*] Description: (.*?)(?=\n\n|\Z)/s, text)
    
    # Pattern 2: Look for function annotations
    docs = if initial_docs == [] do
  ...
  defp extract_usage_examples(text) do
    ...
    interim_examples = if initial_examples == [] do
    ...
    final_examples = if interim_examples == [] do
      ...
      _ ->
        ...
        case Regex.run(~r/## (?:Implementation|Algorithm) Details[\s\n]+(.*?)(?=\n##|\Z)/s, text) do
          ...
          _ ->
            ...
            case Regex.scan(~r/\b(?:algorithm|implementation|approach|method|technique|strategy)\b.*?(?:\.|$)/i, text) do
              [] -> ""
              matches -> 
                matches 
