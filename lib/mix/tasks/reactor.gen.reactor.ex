if Code.ensure_loaded?(Igniter) do
  defmodule Mix.Tasks.Reactor.Gen.Reactor do
    ...
    def igniter(igniter) do
      ...
      if "--ignore-if-exists" in igniter.args.argv_flags && exists? do
        ...
        return_step = Keyword.get(igniter.args.options, :return) || Mix.raise("You must specify a return step with --return")

        inputs_code = generate_inputs(inputs)
        steps_code = generate_steps(steps)

        # Generate Step Modules
        igniter = Enum.reduce(steps, igniter, fn {step_name, module_name}, acc ->
  ...
  defmodule Mix.Tasks.Reactor.Gen.Reactor do
    @shortdoc "Generate a Reactor module using Igniter"
    @moduledoc """
    Generate a Reactor module.
    ...
    def run(_args) do
      Mix.shell().error("""
      The task 'reactor.gen.reactor' requires Igniter to be run.
